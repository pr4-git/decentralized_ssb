...
func NewServer(keyPair handshake.EdKeyPair, appKey []byte) (*Server, error) {
...
func (s *Server) ConnWrapper() netwrap.ConnWrapper {
	return func(conn net.Conn) (net.Conn, error) {
		state, err := handshake.NewServerState(s.appKey, s.keyPair)
		if err != nil {
			return nil, err
		}
		...
		go func() {
			errc <- handshake.Server(state, conn)
			close(errc)
		}()
