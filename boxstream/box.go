...
const (
	// HeaderLength defines the length of the header packet before body
...
// Boxer encrypts everything that's written to it
type Boxer struct {
	l      sync.Mutex
	w      io.Writer
	secret *[32]byte
...
func (b *Boxer) WriteMessage(msg []byte) error {
	if len(msg) > MaxSegmentSize {
		log.Fatalln("message exceeds maximum segment size")
	}
